...
**지우는 순서는 규약입니다.** 굳은 것이 무른 것을 붙잡습니다 — 사진은 그것이 든 데이터셋 버전이,
데이터셋은 거기서 뜬 버전이, 버전은 그것으로 돌린 학습 작업이, 작업은 그 작업이 낸 모델 버전이 붙잡습니다.
막는 곳을 하나 열면 **학습을 돌린 그 판이 재현되지 않고**, 그 사실은 몇 달 뒤 사고를 되짚을 때에야 드러납니다.
모델 버전은 Staging·Production 이면 스테이지를 먼저 내리게 합니다 — 강등이 `PromoteAsync` 를 지나야
누가 언제 왜 내렸는지가 이력에 남습니다. 삭제로 건너뛰지 마세요. Retired 는 Admin 만 지웁니다.
파일은 내용 주소라 **같은 키를 쓰는 다른 행이 없을 때만** 지웁니다
(`ModelRegistryService.DeleteVersionAsync`·`ImagePoolService.DeleteAsync`). `DeletionApiTests` 가 이 순서를 지킵니다.
**나이나 용량으로 사람 없이 지우는 보존 정책은 넣지 마세요** — 되돌릴 수 없고, 무엇을 얼마나 두는지는 운영이 정합니다.
