...
**파이프라인 템플릿.** Phase 5 의 나머지 한 조각입니다. 데이터 수집 → 라벨링 → 학습 → 승격을
정해 둔 순서대로 자동으로 도는 틀이 없습니다. 지금은 사람이 각 단계를 눌러 넘깁니다.
자동으로 도는 것과 사람이 승인하는 것의 경계를 먼저 정해야 해서 남겨 두었습니다 —
Production 승격은 어떤 경우에도 사람이 합니다.
