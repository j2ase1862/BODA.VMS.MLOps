...
객체를 클릭하면 그 경계를 폴리곤으로 만들어 줍니다. 점을 더 찍어 넓히고,
Shift+클릭(또는 오른쪽 클릭)으로 빼고 싶은 곳을 지정합니다. Enter 로 확정해야 라벨이 되고,
그 전까지는 점선 미리보기일 뿐입니다. 검출 데이터셋에서는 폴리곤 대신 사각형으로 받을 수도 있습니다.

모델은 MobileSAM(Apache 2.0)이고 저장소에 넣지 않습니다. 두 가지 방법 중 하나로 준비합니다.

```bash
# 1) VMS 리포에 이미 있는 것을 복사 (4개 파일이 한 세트)
...
경로는 `appsettings.json` 의 `Sam:EncoderPath`·`Sam:DecoderPath` 이고 상대 경로는 설치 폴더 기준입니다.
...
| `POST /api/sam/predict` | 클릭 점들 → 폴리곤·외접 박스 |
...
177개가 GPU 없이 몇 초 만에 끝납니다. 규약 판별은 VMS 리포와 같은 스텁 ONNX 를 씁니다.
...
SAM 은 마스크에서 폴리곤을 뽑는 순수 부분을 모델 없이 확인하고, 모델 파일이 있는 PC 에서는
실제 추론까지 돌려 클릭한 자리에 폴리곤이 생기는지와 두 객체를 갈라 잡는지를 확인합니다.
