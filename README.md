...
301개가 GPU 없이 몇 초 만에 끝납니다. 규약 판별은 VMS 리포와 같은 스텁 ONNX 를 씁니다.

**전체를 돌리면 드물게 한 건이 흔들립니다** — 다섯 번에 한 번쯤, 매번 다른 시험입니다
(이미지 업로드 POST, SAM 추론 500, 내보내기 해시). 단독으로 돌리면 늘 통과하고 재실행해도 통과합니다.
무거운 경로(이미지 디코딩·ONNX 세션)에서만 나는 것으로 보아 병렬 실행 중 자원 경합으로 의심하지만
원인은 아직 좁히지 못했습니다. **어떤 변경 때문이 아닙니다** — 이 글을 쓸 당시 이전 커밋에서도 같은 비율로 났습니다.
한 건이 실패하면 그 시험만 단독으로 다시 돌려 보고, 거기서도 실패하면 그때는 진짜입니다.
