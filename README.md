...
라벨링은 손으로 그리는 것 외에 SAM 클릭 분할도 씁니다. Phase 5(모니터링·재학습 루프)는 아직 없습니다.
...
**VMS 리포 연동.** `model://` 참조 해석기와 로컬 캐시, VisionSetup 의 [레지스트리…] 버튼,
WPF 도구의 업로드·데이터셋 내려받기 버튼. VMS 리포에는 공유 규약 패키지(`VMS.Core.Contracts`)만 있고
이 서버를 부르는 코드가 없습니다. "라인에 실제로 내려간다" 는 절반이 여기 걸려 있습니다.
