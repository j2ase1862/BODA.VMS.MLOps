...
**운영 웹 없이 서는 로그인.** 지금 사람의 인증은 운영 웹이 하고, 그것이 없는 설치에서는
같은 키로 토큰을 손수 만들어 붙여넣는 뒷길뿐입니다. 자체 계정(`Auth:Mode=Local`)을 두 번째 모드로
넣는 설계를 `docs/로컬 계정 로그인 설계 메모.md` 에 적어 두었습니다 — 규약·데이터 모델·시험 목록과,
로컬 토큰을 운영 웹에 물어보면 전원이 끊기는 함정까지. 아직 제안이고 구현하지 않았습니다.
