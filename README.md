...
**사진이 들어오는 길은 둘입니다.** 라인 PC 가 스스로 올리는 것(`POST /api/images/line-ng`)과
사람이 자기 PC 에서 올리는 것입니다. 사람이 올리는 길은 **수집 사진** 화면과 **데이터셋 상세** 화면
양쪽에 있고(같은 `ImageUploadDialog` 컴포넌트), 끌어다 놓거나 골라서 한 번에 500장까지 올립니다.
10장씩 나눠 보내므로 한 묶음이 실패해도 나머지는 올라갑니다. 올라간 사진은 출처만 `manual` 로
다를 뿐 라인 사진과 똑같이 다뤄집니다 — 라벨링·판 뜨기·학습이 그대로입니다.
받는 확장자는 `ImageProcessor.IsSupportedExtension` 한 곳이 정하고, 화면의 파일 고르개가 같은 목록을 내밉니다.
...
**검사할 자리만 잘라 담을 수 있습니다 (ROI 자르기).** 수집 사진과 데이터셋 상세에서 사진을 고르고
`[ROI 로 자르기]` 를 누르면, 한 장에서 정한 자리로 **고른 사진 전부**를 잘라 새 사진으로 담습니다.
같은 치구에서 같은 구도로 찍는 검사 사진이라 한 장에서 정한 자리가 나머지에도 맞습니다.

필요한 이유는 해상도입니다. 원본이 2448×2048 인데 분류·이상탐지 학습의 기본 입력은 224 입니다 —
11배로 줄이면 작은 이물은 한 화소도 남지 않습니다. 검사할 자리만 잘라 두면 살아남습니다.

**원본은 지우지 않습니다.** 자른 것이 원본을 대신하는 것이 아니라 나란히 쌓입니다 — 자리를 잘못
잡았을 때 되돌릴 데가 있어야 하고, 무엇을 지울지는 사람이 정합니다. 출처·라인·검사 이력·태그는
원본에서 물려받습니다. 라인 NG 사진을 잘랐다면 그것은 여전히 그 라인의 그 검사에서 나온 사진입니다.
어디서 어느 자리를 잘랐는지는 `sourceImageId`·`roi` 로 남고, 원본을 지우면 그 연결만 풀립니다
(자른 사진은 그 자체로 온전한 학습 재료라 원본을 붙잡지 않습니다).

잘라낸 사진은 **무손실 PNG** 입니다. JPEG 으로 다시 구우면 매끈한 가공면에 블록 무늬가 생기는데,
이상탐지는 그 무늬까지 "정상의 모습" 으로 배우거나 이상으로 집습니다.

**넘기는 길은 둘이고 뜻이 다릅니다.** `[저장하고 다음]` 은 이 장을 라벨 완료로 찍고 **아직 남은
다음 일**로 뜁니다. 화살표(←/→)와 `[이전]`·`[다음]` 은 라벨 상태와 무관하게 **목록 순서**대로
오갑니다 — 다 라벨링한 뒤에도 되돌아가 고칠 수 있어야 하기 때문입니다. 둘을 같은 것으로 두면
마지막 한 장을 끝내는 순간 화면이 막다른 길이 됩니다. 남은 일이 없으면 `[저장하고 다음]` 이
그렇게 말하고(끝난 것인지, 남은 사진을 남이 잡고 있는 것인지 갈라서) 데이터셋으로 돌아갈 길을 냅니다.
`[라벨링 시작]` 도 다 끝난 데이터셋에서 오류를 내지 않고 첫 장을 엽니다.
자리표는 `GET /api/datasets/{id}/label-queue` 한 곳이 계산합니다.
...
**사람이 들어오는 문은 둘 중 하나입니다 (`Auth:Mode`).** 기본은 `Web` — 운영 웹(BODA.VMS.Web)이
로그인을 확인하고 우리는 그 토큰을 검증합니다. `Local` 은 운영 웹이 없는 설치용으로, 이 서버가
계정과 비밀번호(PBKDF2 해시)를 직접 들고 `Auth:Local:Key` 로 토큰을 발급합니다.
**두 모드는 배타적입니다** — 함께 열면 같은 아이디가 두 출처에서 들어올 수 있고 `Members` 는
계정 이름이 키라 누구인지 가릴 수 없습니다. 설계 판단은 `docs/로컬 계정 로그인 설계 메모.md` 에 있습니다.

로컬 모드에서도 **역할은 `Members` 표가 정합니다.** 로그인 토큰에 자체 발급 표시(`mlops:self`)를
붙이지 않기 때문에 역할 변환이 그대로 돌고, 강등·비활성이 다음 요청부터 듣습니다.
비밀번호를 바꾸면 `SecurityStamp` 가 새로 찍혀 다른 자리의 세션이 즉시 끊깁니다 —
운영 웹이 토큰 세대(`tv`)로 하는 일을 여기서는 그 도장이 합니다.
...
`VMS.Core.Contracts` 1.33.0 패키지부터 반영, 이 리포도 2026-09-10 부터 `1.33.*` 를 참조합니다).
...
351개가 GPU 없이 몇 초 만에 끝납니다. 규약 판별은 VMS 리포와 같은 스텁 ONNX 를 씁니다.

한동안 전체를 돌리면 다섯 번에 한 번쯤 한 건이 흔들렸습니다. 매번 다른 시험이라 병렬 실행의
자원 경합처럼 보였지만, 원인은 하나였습니다 — **사진을 읽은 뒤 파일 손잡이가 늦게 닫혀
임시 파일을 제자리로 옮기지 못하는 것**이었습니다 (아래 "사진 파일은 우리가 열고 우리가 닫습니다").
고친 뒤 20회 연속 통과했습니다. 그 두 자리를 지키는 시험이
`ImageProcessorHandleTests` 와 `ArtifactStorageTests` 입니다.
...
**운영 웹 없이 서는 로그인 — 1단계가 들어갔습니다.** `Auth:Mode=Local` 이면 이 서버가 계정과
비밀번호를 직접 들고, 설치 스크립트의 `-Local` 이 서명 키와 첫 관리자를 만듭니다.
