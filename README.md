...
217개가 GPU 없이 몇 초 만에 끝납니다. 규약 판별은 VMS 리포와 같은 스텁 ONNX 를 씁니다.
...
WPF 도구의 업로드·데이터셋 내려받기 버튼. VMS 의 검사 도구는 지금 `ModelPath` 에 절대 파일 경로를
그대로 담고, 그 문자열이 곧바로 `InferenceSession` 으로 들어갑니다. 승격한 모델을 쓰려면
사람이 파일을 복사하고 경로를 다시 입력해야 합니다. "라인에 실제로 내려간다" 는 절반이 여기 걸려 있습니다.
서버 쪽 준비는 끝났습니다 — 라인 토큰으로 `resolve` 와 아티팩트 내려받기가 됩니다.
