...
사람은 운영 웹(BODA.VMS.Web)에서 받은 JWT 로 들어옵니다. 운영 웹의 역할은 Admin · User · Guest 셋이라
MLOps 역할과 겹치는 것이 Admin 하나뿐이므로, 서버가 토큰을 검증한 직후 이렇게 옮깁니다.

| 운영 웹 | MLOps | 근거 |
|---|---|---|
| Admin | Admin | 이름이 같다 |
| User | **Engineer** | 운영 웹 User 는 엔지니어급이다 — VMS 에 SSO 로 들어가면 Engineer 로 매핑되어 운영 중인 레시피를 고친다 |
| Guest | **Viewer** | 조회만 하는 계정 |
| 그 밖 | (없음) | 로그인이 403 으로 거부된다 |

원래 역할은 지우지 않고 곁에 더합니다. User 가 Engineer 가 된다는 것은 모델 등록·학습 제출·Staging 승격과
**Staging 판을 레시피에 고정 바인딩해 라인에 거는 것**까지입니다. Production 승격은 여전히 Admin 만 합니다.

운영 웹 쪽에 역할을 더하지 않은 이유: 운영 웹은 GS 인증 범위 안이고 MLOps 는 밖입니다(인증 목표 2026-10-30).
라벨링만 하는 계정이 필요해지면 MLOps 쪽에 사람별 역할을 두는 것으로 풉니다.
로그인 절차는 `docs/로그인 가이드.md` 를 보세요.
