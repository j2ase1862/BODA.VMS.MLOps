...
라벨링 화면은 **미라벨 이미지가 남아 있는 데이터셋**을 골라야 "라벨 없음" 상태의 캔버스가 찍힙니다
(다 라벨링된 데이터셋도 열리기는 합니다 — 첫 장이 열립니다). 사진이 아예 없는 데이터셋만
빈 화면입니다. `capture.js` 가 검출·분류에서 각각 미라벨이 남은 데이터셋을 골라 씁니다.
